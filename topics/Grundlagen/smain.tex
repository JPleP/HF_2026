\section{Grundlagen}
\renewcommand{\bildf}[1]{topics/Grundlagen/#1}

\Term{Mikrostreifenleitungen}:
Quasi-TEM-Wellen
\begin{tikzpicture}
    \draw (0,0) rectangle (2,0.3) node[midway] {$\varepsilon_r$};
    \filldraw (0,0) rectangle (2,-0.1) node[midway, below] {Masse};
    \filldraw (0.9,0.3) rectangle (1.1,0.35) node[at end, above] {Luft};
\end{tikzpicture}

\Term{Triplate Leitung}:
TEM-Wellen

\begin{tikzpicture}
    \draw (0,0) rectangle (2,0.3);
    \filldraw (0,0) rectangle (2,-0.1) node[midway, below] {Masse};
    \filldraw (0,0.3) rectangle (2,0.4);
    \filldraw (0.9,0.12) rectangle (1.1,0.18);
\end{tikzpicture}




\Term{Leitungsgleichung}
$\gamma, Z_L$

$U = U_f \e^{- \gamma z} + U_b \e^{\gamma z}$\\
$I = \frac{U_f}{Z_L} \e^{-\gamma z} - \frac{U_f}{Z_L} \e^{\gamma z}$

Normierungs:

$u = \frac{U}{\sqrt{Z_L}}$ , $i = I \sqrt{Z_0}$ , $U_f = I_f Z_L$

$u = u_f\e^{-\gamma z} + u_b\e^{\gamma z} = a + b$\\
$i = a-b$


\Term{Matrixbeschreibungen}
\begin{pitemize}
    \item \Term{S-Parameter (Streuparameter)}:
    \begin{pitemize}
        \item Beschreibung über Wellen ($a_i$ (Eingang Oben), $b_i$ (Ausgang unter), Nummer sind die Seite)
        \item $\begin{pmatrix} b_1 \\ b_2 \end{pmatrix}
                =
                \begin{pmatrix} S_{11} & S_{12} \\ S_{21} & S_{22} \end{pmatrix}
                \begin{pmatrix} a_1 \\ a_2 \end{pmatrix}$
            \begin{pitemize}
                \item $S_{11} \big|_{a_2 = 0} = \frac{b_1}{a_1}$: Reflexion Eingang
                \item $S_{21} \big|_{a_2 = 0} = \frac{b_2}{a_1}$: Transmission (Verstärkung)
                \item $S_{12}$: Rückwirkung
                \item $S_{22}$: Reflexion Ausgang
            \end{pitemize}
        \item $\frac{\begin{pmatrix}
            A + B - C - D & 2\Delta_A \\
            2  & -A + B - C +D
        \end{pmatrix}}{A+B+C+D}$
        \item $\frac{\begin{pmatrix}
            \Sigma_{12} & \Delta_\Sigma \\
            1  & -\Sigma_{21}
        \end{pmatrix}}{\Sigma_{22}}$
        \item \includegraphics[width=0.5\linewidth]{topics/Grundlagen/Streuparameter.jpeg}
    \end{pitemize}

    \item \Term{Impedanzparameter}:
    \begin{pitemize}
        \item $U_1 = Z_{11}I_1 + Z_{12}I_2$
        \item $U_2 = Z_{21}I_2 + Z_{22}I_2$
        \item \includegraphics[width=0.5\linewidth]{topics/Grundlagen/Impedanzparameter}
    \end{pitemize}

    \item \Term{Kettenparameter}:
    \begin{pitemize}
        \item Darstellung der ABCD-Parameter
        \item $\begin{pmatrix} U_1 \\ I_1 \end{pmatrix}
            =
            \begin{pmatrix} A & B \\ C & D \end{pmatrix}
            \begin{pmatrix} U_2 \\ -I_2 \end{pmatrix}$
        \item $\frac{\begin{pmatrix}
            -\Delta_S + S_{11} - S_{22} + 1 & \Delta_S + S_{11} + S_{22} + 1 \\
            \Delta_S - S_{11} - S_{22} + 1 & -\Delta_S - S_{11} + S_{22} + 1
        \end{pmatrix}}{2S_{21}}$
        \item \includegraphics[width=0.5\linewidth]{topics/Grundlagen/Kettenparameter.jpeg}
    \end{pitemize}

    \item \Term{Transmissionsmatrix}:
    \begin{pitemize}
        \item Multiplikation Vereinfachung
        \item $\begin{pmatrix} b_1 \\ a_1 \end{pmatrix}
                =
                \begin{pmatrix} \Sigma_{11} & \Sigma_{12} \\ \Sigma_{21} & \Sigma_{22} \end{pmatrix}
                \begin{pmatrix} a_2 \\ b_2 \end{pmatrix}$
        \item $[\Sigma] = \frac{1\begin{pmatrix}
            -\Delta_S & S_{11} \\ -S_{22} & 1
        \end{pmatrix}}{S_{21}}$
        \item \includegraphics[width=0.5\linewidth]{topics/Grundlagen/Transmissionsparamter}
        \item \includegraphics[width=0.5\linewidth]{topics/Grundlagen/Hintereinanderschaltung.jpeg}
        \item $\left[\Sigma_g\right] =  \left[\Sigma_1\right] \left[\Sigma_2\right]$   
    \end{pitemize}
    
\end{pitemize}

\subsection{Leitungen}

\begin{tikzpicture}
    \draw (0.33,-0.05) rectangle (0.66,0.05);
    \draw (0,0) -- (0.33,0); \draw (0.66,0) -- (1,0);
    \draw (0,-0.5) -- (1,-0.5);
\end{tikzpicture}
$\begin{bmatrix}
    A & B \\ C & D
\end{bmatrix} =
\begin{bmatrix}
    1 & \frac{R}{Z_0} \\0 & 1
\end{bmatrix}$ 

\begin{tikzpicture}
    \draw (0.45,-0.1) rectangle (0.55,-0.4);
    \draw(0.5,0) -- (0.5,-0.1);
    \draw(0.5,-0.5) -- (0.5,-0.4);
    \draw (0,0) -- (1,0);
    \draw (0,-0.5) -- (1,-0.5);
\end{tikzpicture}
$\begin{bmatrix}
    A & B \\ C & D
\end{bmatrix} =\begin{bmatrix}
    1 & 0 \\ GZ_0 & 1
\end{bmatrix}$

\subsubsection{Dämpfungsglied}
\includegraphics[width=0.5\linewidth]{topics/Grundlagen/Bsp Dämpfungsglied.jpeg}
$\begin{bmatrix}
    A & B \\ C & D
\end{bmatrix} = \begin{bmatrix}
    1 & 0 \\ G_1Z_0 & 1
\end{bmatrix}\begin{bmatrix}
    1 & \frac{R_2}{Z_0} \\ 0 & 1
\end{bmatrix}\begin{bmatrix}
    1 & 0 \\ G_1Z_0 & 1
\end{bmatrix}$
